...
import com.minecraftwithcommands.CommandLine;
...
public class Game {
    ...
    public void start() {
        ...
        Thread commandLineThread = new Thread(() -> {
            while (true) {
                CommandLine.CommandType command = commandLine.getLastCommand();
                switch (command) {
                    case MOVE_UP -> player.Move(Direction.UP);
                    case MOVE_DOWN -> player.Move(Direction.DOWN);
                    case MOVE_FORWARD -> player.Move(Direction.FORWARD);
                    case MOVE_BACK -> player.Move(Direction.BACK);
                    case MOVE_RIGHT -> player.Move(Direction.RIGHT);
                    case MOVE_LEFT -> player.Move(Direction.LEFT);
                    case ROTATE_UP -> player.Rotate(Direction.UP);
                    case ROTATE_DOWN -> player.Rotate(Direction.DOWN);
                    case ROTATE_RIGHT -> player.Rotate(Direction.RIGHT);
                    case ROTATE_LEFT -> player.Rotate(Direction.LEFT);
                    case DESTROY_BLOCK -> player.destroyBlock(world);
                    case PLACE_BLOCK -> player.placeBlock(world);
                    case CHANGE_BLOCK -> player.changeBlockToPlace(world);
                }
                try {
                    Thread.sleep(200);
